...
string fileName;
GLfloat myColorBlock[736][551][3];
void pixelMap(){
	cout << "Loading... Please be patient";
	fileName = "\\\\trace\\Class\\CSC-525-625\\001\\Cristian9\\projects\\project1Redo\\image123.txt";
	...
	if (fileOpened.is_open()){
		...
		for (int x = 0; x < 736; x++){
			for (int z = 0; z < 551; z++){
				...
				fileOpened >> g;
				fileOpened >> b;
				myColorBlock[x][z][0] = r;
				myColorBlock[x][z][1] = g;
				myColorBlock[x][z][2] = b;
			}
		}
		cout << "finish";
	}
...
{
	//pixelMap();
	//glDrawPixels(736, 551, GL_RGB, GL_FLOAT, myColorBlock);
	/*cout << endl << "Writing RGB vals to a new file" << endl;
	cout << endl << "This might take a few minutes" << endl;
	ofstream fileWritten("C:\\TEMP\\savedImg.txt");
	for (int y = 0; y < 512; y++){
		for (int x = 0; x < 512; x++){
			glReadPixels(x, y, 1, 1, GL_RGB, GL_FLOAT, myColorBlock);
			fileWritten << myColorBlock[0] << " " << myColorBlock[1] << " " << myColorBlock[2] << " ";
		}
	}
	cout << "pixels written to file";
	fileWritten.close();*/
